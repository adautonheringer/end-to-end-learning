...
class World(object):
    ...
    def __init__(self, carla_world, hud, args):
        ...
        # Town10HD
        # self.start_location = carla.Location(x=-15.148191, y=69.714005, z=0.600000)
        # self.end_location = carla.Location(x=74.798752, y=28.343533, z=0.600000)
        # Town02
        # self.start_location = carla.Location(x=104.300018, y=241.280029, z=0.500000)
        # self.end_location = carla.Location(x=166.914505, y=191.770035, z=0.500000)
        # Town02 Longer Route
        self.start_location = carla.Location(x=-7.530004, y=234.110001, z=0.500000)
        self.end_location = carla.Location(x=166.914505, y=191.770035, z=0.500000)
        self.restart(args)
    ...
    def restart(self, args):
        ...
        while self.player is None:
            ...
            transform = carla.Transform(self.start_location, carla.Rotation(yaw=90))
...
class CameraManager(object):
    ...
    def __init__(self, parent_actor, hud):
        """Constructor method"""
        self.model = load_model('../CNN_Models/dataset_free_autopilot_1hour_noise_norain-08-0.0121.hdf5')
        self.sensor = None
        self.surface = None
        self._parent = parent_actor
        self.hud = hud
    ...
    def set_sensor(self, index, agent: agents.navigation.basic_agent.BasicAgent = None, notify=True, force_respawn=False,
                   traffic_manager: carla.TrafficManager = None, blueprint = None):
        """Set a sensor"""
        ...
        if needs_respawn:
            ...
            # self to avoid circular reference.
            weak_self = weakref.ref(self)
            self.sensor.listen(lambda image: CameraManager._parse_image(weak_self, image, agent, traffic_manager))
        if notify:
    ...
    @staticmethod
    def _parse_image(weak_self, image, agent: agents.navigation.basic_agent.BasicAgent, traffic_manager: carla.TrafficManager):
        self = weak_self()
        array = None
        if not self:
            return
        if self.sensors[self.index][0].startswith('sensor.lidar'):
            ...
            # resized = cv2.resize(array, dsize=(180, 300))

            # reshaped_image = array.reshape(-1, 180, 300, 3)
            ...
            if agent is not None:
                ...
                prediction = self.model.predict([reshaped_image, reshaped_hlc])

                print(prediction)

                control = carla.VehicleControl()
                control.steer = float(prediction[0][0])
                control.throttle = float(prediction[0][1])
                control.brake = 0
...
def game_loop(args):
    ...
    try:
        ...
        # start_spawn_point = random.choice(spawn_points) if spawn_points else carla.Transform()
        # end_spawn_point = random.choice(spawn_points) if spawn_points else carla.Transform()
        ...
        agent.set_destination(end_location=world.end_location)
        ...
        camera_manager.set_sensor(0, agent=agent, traffic_manager=traffic_manager, blueprint=front_camera_bp)
        # camera_manager.toggle_recording()

        count = 0
        # sim_world.set_weather(weather_preset[random.randint(0, 14)])
        sim_world.set_weather(weather_preset[1])

        # route = ["Right", "Straight", "Right", "Right", "Left", "Right"]
        # traffic_manager.set_route(world.player, route)
        ...
        while True:
            ...
            if False:
                if args.loop:
                    if count == 5:
                        ...
                        break
                    if where_to == world.location_3:
                        count += 1

                    where_to = world.location_1 if where_to == world.location_3 else world.location_3
            ...
            # print(agent.get_local_planner().get_incoming_waypoint_and_direction(0)[1].value)
            # control.manual_gear_shift = False
